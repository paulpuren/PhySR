...
class ShiftMean(nn.Module):
    ...
    def __init__(self, mean, std):
        ...
        self.mean = torch.Tensor(mean).view(1, 1, 4, 1, 1)
        self.std = torch.Tensor(std).view(1, 1, 4, 1, 1)
...
class RBDataset(Dataset):
    def __init__(self, data_dir):
        ...
        # read data
        data = np.load(self.data_dir + 'rb2d_ra1e6_s42_dt0.01.npz', mmap_mode='r')
        p = data['p'] # [200,512,128]
        T = data['b'] 
        u = data['u']
        v = data['w']
        hres = np.concatenate((p[:,None,...], T[:,None,...], u[:,None,...], v[:,None,...]), axis=1)
        hres = hres[2500:2500+1600,...]  # [1600,4,512,128]
        print('selected hres time sequence length: ', hres.shape[0])
        lres = hres[::4, :, ::8, ::8] # [400,4,64,16]
        hres = torch.tensor(hres, dtype=torch.float32)
        lres = torch.tensor(lres, dtype=torch.float32)
        
        # split the data segments
        for j in range(int(hres.shape[0]/32)): # temporal, 1600/16=100
            self.samples.append((lres[8*j:8*(j+1),:,:,:], hres[32*j:32*(j+1),:,:,:]))                
...
if __name__ == '__main__':
    ...
    data_dir = './data/2DRB/'
    data_loader = RBDataset(data_dir)
    ...
    save_path = './model/2DRB[a=1,b=0.025,len=32,channel32,512x128]/'
